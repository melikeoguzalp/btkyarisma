...
export default function GamePage() {
    ...
    const fetchNewScenarios = async (profile: UserProfile, count: number, lang: 'en' | 'tr') => {
        ...
        try {
            ...
            const scenarioPromises = Array(count).fill(null).map(() => generateGameScenario({
            ...
            console.error(err);
            const errorMsg = language === 'tr' ? 'Yeni oyun senaryoları yüklenemedi. Lütfen daha sonra tekrar deneyin.' : 'Failed to load new game scenarios. Please try again later.';
            setError(errorMsg);
        } finally {
            setIsLoading(false);
        }
    ...
    const handleAnalysisSubmit = async () => {
        ...
        try {
            ...
            console.error(err);
            const errorMsg = language === 'tr' ? 'Analiz değerlendirilemedi. Lütfen tekrar deneyin.' : 'Failed to evaluate analysis. Please try again.';
            setError(errorMsg);
        } finally {
        ...
        return (
            <Card className="shadow-lg animate-in fade-in bg-card/80 backdrop-blur-sm">
                <CardHeader>
                    ...
                    <CardDescription>
                        {isCorrectCategory
                            ? (language === 'en' ? `You earned ${earned} points!` : `${earned} puan kazandın!`)
                            : (language === 'en' ? 'You didn\'t earn any points this round, but let\'s see why.' : 'Bu tur hiç puan kazanamadın, ama nedenine bakalım.')
                        }
                    </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-center">
                        <div className="p-4 rounded-lg bg-background/50 border">
                            <p className="text-sm text-muted-foreground">{language === 'en' ? 'Your Score' : 'Senin Puanın'}</p>
                            <p className="text-3xl font-bold">{userRiskScore}</p>
                        </div>
                        <div className={cn("p-4 rounded-lg border", isCorrectCategory ? "border-green-500 bg-green-500/10" : "border-destructive bg-destructive/10")}>
                            <p className="text-sm text-muted-foreground">{language === 'en' ? 'AI Tutor\'s Score' : 'Yapay Zeka Eğitmeninin Puanı'}</p>
                    ...
                    <Accordion type="single" collapsible defaultValue="feedback">
                        <AccordionItem value="feedback">
                            <AccordionTrigger>{language === 'en' ? 'Detailed Feedback' : 'Detaylı Geri Bildirim'}</AccordionTrigger>
                            <AccordionContent className="space-y-4">
                               <p className="text-base">{result.feedback}</p>
                               <Alert variant={isCorrectCategory ? "default" : "destructive"}>
                                   <AlertCircle className="h-4 w-4" />
                                   <AlertTitle>{language === 'en' ? 'AI\'s Reasoning' : 'Yapay Zekanın Gerekçesi'}</AlertTitle>
        ...
        <main className="container mx-auto p-4 md:p-8">
            <header className="mb-8">
                ...
                <p className="text-muted-foreground mt-2">
                    {language === 'en' ? 'Analyze the scenario and test your skills!' : 'Senaryoyu analiz et ve yeteneklerini sına!'}
                </p>
            </header>
            ...
            <div className="w-full max-w-3xl mx-auto">
                        ...
                        <CardHeader>
                            <div className="flex justify-between items-center">
                                {currentItem.type === 'email' ? (
                                    <div className="flex items-center gap-2 text-muted-foreground"><Mail className="h-5 w-5" /><span>{language === 'en' ? 'Email' : 'E-posta'}</span></div>
                                ) : (
                                    <div className="flex items-center gap-2 text-muted-foreground"><MessageSquare className="h-5 w-5" /><span>SMS</span></div>
                                )}
                                <span className="text-sm text-muted-foreground">{language === 'en' ? 'From' : 'Gönderen'}: {currentItem.sender}</span>
                            </div>
                            {currentItem.subject && (<CardTitle className="mt-2 text-xl">{currentItem.subject}</CardTitle>)}
                        </CardHeader>
                        <CardContent className="text-base flex-grow"><p className="whitespace-pre-wrap">{currentItem.content}</p></CardContent>
                        <CardFooter className="flex flex-col gap-6">
                            <div className="w-full space-y-4">
                                <div>
                                    <Label htmlFor="reasoning" className="font-semibold">{language === 'en' ? 'Your Analysis' : 'Senin Analizin'}</Label>
                                    <Textarea
                                        id="reasoning"
                                        value={userReasoning}
                                        onChange={(e) => setUserReasoning(e.target.value)}
                                        placeholder={language === 'en' ? 'Why do you think this is risky or safe? (e.g., "The link looks suspicious, it creates a sense of urgency...")' : 'Sizce bu neden riskli veya güvenli? (ör: "Bağlantı şüpheli görünüyor, aciliyet hissi yaratıyor...")'}
                                        className="mt-2"
                                    />
                                </div>
                                <div>
                                <Label htmlFor="risk-score" className="font-semibold">{language === 'en' ? `Your Risk Score Estimate: ${userRiskScore}` : `Tahmini Risk Puanın: ${userRiskScore}`}</Label>
                                <Slider
                                    id="risk-score"
                                    min={0}
                                    max={100}
                ...
                {!currentItem && !isLoading && (
                    <p className="text-center text-muted-foreground">{language === 'en' ? 'You have completed all scenarios!' : 'Tüm senaryoları tamamladınız!'}</p>
                )}
            </div>
        </main>
    )
